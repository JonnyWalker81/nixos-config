  ...
  (orglife-test-with-temp-home
   ...
   (let ((dispatched-key nil)
     ...
     (cl-letf (((symbol-function 'require)
               ...
               ((symbol-function 'org-super-agenda-mode)
                (lambda (&optional arg)
                  (setq mode-arg arg)))
...
(ert-deftest orglife-dashboard-registers-widgets-startup-and-refresh-wiring ()
  (orglife-test-with-temp-home
   ...
   (orglife-test-load "users/doom.d/config-org-integration.el")
    (should (equal initial-buffer-choice #'org-life-dashboard-initial-buffer-choice))
   (should (equal
  ...
  (orglife-test-with-temp-home
   ...
   (let (capture-policy daily-policy weekly-policy roam-policy)
      (cl-letf (((symbol-function 'org-life-dashboard--with-display-policy)
               ...
               ((symbol-function 'org-life-agenda-daily-review)
                (lambda ()
                  (interactive)
                  (setq daily-policy (or daily-policy 'missing))))
               ((symbol-function 'org-life-agenda-weekly-review)
                (lambda ()
                  (interactive)
                  (setq weekly-policy (or weekly-policy 'missing))))
