...
, kernel, libsOnly ? false, fetchurl, undmg, perl, autoPatchelfHook, gnome2
, fuse, }:
...
stdenv.mkDerivation rec {
  ...
  runtimeDependencies = [ glib xorg.libXrandr gnome2.GConf ];
  ...
  kernelDir =
    lib.optionalString (!libsOnly) "${kernel.dev}/lib/modules/${kernelVersion}";
          ...
          if test -f "$out/bin/$tool"; then
            wrapProgram "$out/bin/$tool" \
              --prefix LD_LIBRARY_PATH ':' "${
                lib.makeLibraryPath [ glib gnome2.GConf xorg.libXrandr ]
              }"
          fi
        done
